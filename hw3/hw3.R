...
states <- read.table(file = "states.txt", header = T, sep = '\t', 
...
# Work for Problem #4
states$region <- factor(states$region, levels = c("North Central", "Northeast", "South", "West"))
print("Answer for #4:")
print(states$region)

# Work for Problem #5
annotations <- read.table("PZ.annot.txt", sep = "\t", stringsAsFactors = F)
